function [numopen, denopen, denclsd] = frqpi(num, den)
% Hadi Saadat,  1998

%discr=[
%'                                                                           '
%'  The function [numopen,denopen,denclsd]=frqpi(num, den) is used for the   '
%'  frequency response design of a PI controller.    num & den are           '
...
wmn=w1/10; dw2=w1/10;
while -(sin(thtar)*w1)/M < 0 | cos(thtar) < 0  & w1 > wmn
  w1=w1-dw2;
  [M,ph]=bode(num, den, w1);  % Returns the mag. and phase of G(w)H(w1)
  thta=-180 + pm - ph;
  thtar=thta*pi/180;
...
rreal=real(r);
  for l=1:n-1
  if rreal(l) >=0
  fprintf('   Root on the RHP, system is unstable. Change Phase Margin & repeat.\n\n')
  else,end
  end
